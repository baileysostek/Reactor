...
bool	GLMDetectScaledResolveMode( uint osComboVersion, bool hasSLGU )
{
	...
	// note this function assumes a current context on the graphics.renderer in question
...
GLMRendererInfo::~GLMRendererInfo( void )
{
	if (m_displays)
	{
		// delete all the new'd graphics.renderer infos that the table tracks
...
void	GLMRendererInfo::PopulateDisplays( void )
{
	...
	for( int i=0; i<32; i++)
	{
		// check mask to see if the selected display intersects this graphics.renderer
...
{
	GLMPRINTF(("\n     #%d: GLMRendererInfo @ %08x, graphics.renderer-id=%s(%08x)  display-mask=%08x  vram=%dMB",
		which, this,
...
void	GLMDisplayDB::PopulateRenderers( void )
{
		...
		if (selected)
		{
			...
			if (!cgl_err)
			{
				...
				for( int j=0; j<nrend; j++)
				{
					...
					// early out if graphics.renderer ID already in the table
					...
					// decide if this graphics.renderer goes in the table.
					...
					if (selected)
					{
						...
						// iterate through IOAccelerators til we find one that matches the vendorid and deviceid of this graphics.renderer (ugh!)
							...
							case	0x1002:	//ATI
							{
								...
								if ( (fields.m_pciDeviceID >= 0x6898) && (fields.m_pciDeviceID <= 0x68Be) )
								{
									fields.m_atiR8xx = true;
										...
										// we could also use the bit encoding of the graphics.renderer ID to ferret out a geberation clue.
					...
					// we need something here that will exclude the graphics.renderer if it does not have any good displays attached.
	...
	// then go back and ask each graphics.renderer to populate its display info table.
	for( std::vector< GLMRendererInfo * >::iterator p = m_renderers->begin(); p != m_renderers->end(); p++ )
	{
		(*p)->PopulateDisplays();
	}
...
void	GLMDisplayDB::Populate(void)
{
	...
	// passing in zero here, constrains the set of fake adapters (GL graphics.renderer + a display) to the ones using the highest ranked graphics.renderer.
