...
import scripting.JavaScript.Console;
...
public class ScriptingEngine{
    ...
    private ScriptingEngine(){
        this.put("console", Console.class, new Console());
        this.put("Math", java.lang.Math.class);
    }
...
//        System.out.println((Object)refrenceInstance);
    ...
    public Object run(Script script, String functionName, Object ... callingParams){
        if(script.functions.containsKey(functionName)){
            ...
            return evalTokens(script, scope, function.body_segments);
    ...
    public Object evalTokens(Script script, Scope scope, LinkedList<TokenCodePair>[] bodySegments ){
        LinkedList<Object> outObjects = new LinkedList<>();
        for(int l = 0; l < bodySegments.length; l++) {
            ...
            switch (action.getToken()) {
                case RETURN: {
                    Object out = resolveToken(script, scope, getSubList(bodySegment, 1, bodySegment.size()));
                ...
                case CALL_TO_FUNCTION: {
                    ...
                    Object out = run(script, action.code, resolvedParams);
                    outObjects.addLast(out);
                    break;
                }
...
//                                    System.out.println("Creating new instance of a:"+bodySegment.get(5).getCode());
                            ...
                            scope.addVar(varName.code, resolveToken(script, scope, getSubList(bodySegment, 3, bodySegment.size())));
                        }
...
//                                    System.out.println("Class:"+clazz.instance);
                                        try {
                                            Object out = method.invoke(value, object1);
                                            outObjects.addLast(out);
                                        } catch (IllegalAccessException e) {
                                            e.printStackTrace();
                                        } catch (InvocationTargetException e) {
        ...
        return outObjects.size() > 0 ? outObjects.toArray()[0] : "void";
    }
...
//        System.out.println();
    ...
    public  Object resolveToken(Script callingScript, Scope scope, LinkedList<TokenCodePair> line){
        ...
        switch (data.getToken()){
            ...
            case CALL_TO_FUNCTION:{
                String callRecord = "";
                callRecord+=data.getCode();
                //Resolve all methods this should be an ld function
                Object[] resolvedParams =  evaluateParens(callingScript, scope, line, 1);
                callRecord+="( ";
                for(int i = 0; i < resolvedParams.length; i++){
                    callRecord+=(ConsoleColors.GREEN + resolvedParams[i] + ConsoleColors.RESET+ (i < resolvedParams.length-1 ? ", " : " "));
                }
                callRecord+=")";

                System.out.println(callRecord);

                Object result = ScriptingEngine.getInstance().run(callingScript, data.getCode(), resolvedParams);

                System.out.println("Function should return:"+result);

                //Parse to correct class type as well
                return result;
            }
            default:{
                System.out.println(scope.containedInScope(data.getCode()));
    ...
    private Object invokeMethod(ExpandedClass clazz, String methodName, Object invoker, Object[] params){
        ...
        try {
            Object out = clazz.invoke(methodName, invoker, params);
            return out;

        }catch (NoSuchMethodException e) {
            System.out.println(ConsoleColors.RED + "Method does not exist:" + methodName+")" + ConsoleColors.RESET);
            e.printStackTrace();
        }
        return null;
    }

    //This function takes in a list of tokens and picks out the tokens inside the params section of a function.
    //For Example, (par1, par2, par3) -> [par1, par2, par3]
    private static LinkedList<TokenCodePair> determineParams(LinkedList<TokenCodePair> bodySegment){
        LinkedList<TokenCodePair> out = new LinkedList<TokenCodePair>();
        int count = 0;
        boolean startFound = false;
        for(TokenCodePair token : bodySegment){
...
//    //Cut out the tokens in between the next first param start and end section
//    private static LinkedList<TokenCodePair> determineParams(LinkedList<TokenCodePair> bodySegment) {
//
//    }
    ...
    private static Object resolveMethodFromClass(Script script, Scope scope, LinkedList<TokenCodePair> bodySegment){
        ...
        while(!foundFunction){
            ...
            if(thisToken.getToken().equals(EnumTokenType.PROP_OF)){
                System.out.println("Beanz:"+nextToken.getCode());
                Object[] params = ScriptingEngine.getInstance().evaluateParens(script, scope, ScriptingEngine.getInstance().getNextParens(bodySegment, index+1), 0);
                ...
                }else if(clazz.methods.containsKey(nextToken.getCode())){
                    System.out.println("Method:"+nextToken.getCode());
                    try {
                        clazz.invoke(nextToken.getCode(), staticClassInstances.get(bodySegment.get(0).getCode()), params);
                    } catch (NoSuchMethodException e) {
                        e.printStackTrace();
                    }
                }
...
//            }
        ...
        for(int i = start; i < bodySegment.size(); i++){
            ...
            if(i == start){
                ...
                if (opIndex >= 0) {
                    ...
                    System.out.println("No Params were evaluated");
                }
            }
    ...
    //Cut up them tokens, make sure to use addlast, to not flip the list.
    public LinkedList<TokenCodePair> getSubList(LinkedList<TokenCodePair> list, int start, int end){
        LinkedList<TokenCodePair> out = new LinkedList<>();
        for(int i = start; i< end ; i++){
            out.addLast(list.get(i));
        }
        return out;
    }
