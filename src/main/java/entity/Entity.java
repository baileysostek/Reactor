...
public class Entity{
    ...
    public float[] getTransform(){
        ...
        MatrixUtils.scaleM(transform, 0, scale, scale, scale);
        transform = MatrixUtils.rotateM(transform, 0,  rotation.z(), 0f,0f, 1f);
        transform = MatrixUtils.rotateM(transform, 0,  rotation.y(), 0f,1f, 0f);
        transform = MatrixUtils.rotateM(transform, 0,  rotation.x(), 1f,0f, 0f);
        MatrixUtils.translate(transform, position);

        if(this.parent != null){
            float[] tmp = MatrixUtils.getIdentityMatrix();
            tmp = MatrixUtils.multiplyMM(tmp, this.parent.getTransform(), 0, transform, 0);
            transform = tmp;
        }
